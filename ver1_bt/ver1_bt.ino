...
void loop()
{
  ...
  if ((value == 0) && (digitalRead (trai) == 0)) {
    ...
    lcd.print(startus);
    EEPROMWritelong(address, 3600); delay(5); //5400
  }
    ...
    for ( i = 0; i <= 30; i++) {
      ...
      if ((digitalRead (trai) == 0) && (digitalRead(bep) == 0)) {
        ...
        if ((reset > 10) && (reset < 29)) {
          strcpy(startus, "Reset "); lcd.setCursor(8 , 2); lcd.print("             ");
          lcd.setCursor(9 , 2); lcd.print(startus); lcd.setCursor(9 , 3); lcd.print("0");
