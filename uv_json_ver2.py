...
if data > 0:
   status = True
   check = True
ser = serial.Serial(port='COM13', baudrate=9600, parity=serial.PARITY_NONE, #port='/dev/ttyUSB0'
                            stopbits=serial.STOPBITS_ONE, bytesize=serial.EIGHTBITS, timeout=1)
try:
   while True:
      #   s = ser.readline()  # Cho doi (timeout) de doc du lieu tu Port noi tiep
      #   data = s.decode()  # Giai ma chuoi du lieu
      ser.write(str(input("1")))                  # write position to serial port
        # and the pysthon script cant read the code below this line
      reachedPos = str(ser.readline())            # read serial port for arduino echo
